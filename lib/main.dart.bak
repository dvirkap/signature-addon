                      ...
                      onDocumentLoaded: (PdfDocumentLoadedDetails details) {
                        ...
                        if (_restorePageState) {
                          ...
                          WidgetsBinding.instance.addPostFrameCallback((_) {
                            if (mounted) {
                              _pdfViewerController.zoomLevel = _targetZoom;
                              Future.delayed(const Duration(milliseconds: 100), () {
                                if (mounted && _targetScrollX != null && _targetScrollY != null) {
                                  _pdfViewerController.jumpTo(
                                    xOffset: _targetScrollX!,
                                    yOffset: _targetScrollY!,
                                  );
                                }
                              });
                            }
...
class _SignatureCropEditorScreenState extends State<SignatureCropEditorScreen> {
  ...
  Future<void> _initImage() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('שגיאה בטעינת התמונה: $e')),
        );
        Navigator.pop(context);
      }
    }
  ...
  void _onSave() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('שגיאה בשמירת התמונה: $e')),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('עריכה וניקוי רקע החתימה'),
        centerTitle: true,
        actions: [
